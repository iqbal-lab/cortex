...
{
    print_to_progressfile($progress_fh, "INITIAL_CHECKS", "FAIL", "Either your ploidy is mis-specified (should be 1 or 2), or your --ref argument is wrong (should be Absent, CoordinatesOnly, or CoordinatesAndInCalling\n", "");
...
if (!(-e $classifier))
{
    print_to_progressfile($progress_fh, "INITIAL_CHECKS", "FAIL", "Cannot find $classifier - should be in $analyse_variants_dir", "");
...
sub get_num_kmers_for_pool_at_specific_k
{
	...
	print_to_progressfile($progress_fh, "CALC_TOTAL_KMERS_IN_POOL", "FAIL", "Cannot create this file $list_all_clean_path needed internally. Very odd - out of disk?", "");
...
sub get_refbin_name
{
    ...
    opendir (DIR, $dir) or print_to_progressfile($progress_fh, "CALLING_PREP", "FAIL", "Cannot open reference binary directory $dir", "");
    my @files=();
    ...
    {
	print_to_progressfile($progress_fh, "CALLING_PREP", "FAIL", "Cannot find a reference binary for k=$kHHmer in the specified dir $dir", "");
    }
    elsif ($found>1)
    {
...
sub get_colourlist_for_joint
{
    ...
    open(OUT, ">".$outfile)||print_to_progressfile($progress_fh, "CALLING_PREP", "FAIL", "Cannot open $outfile to make a colourfile", "");
