...
##   cat classifier.parallel.correct.R | R --vanilla --args arg1 arg2 arg3 arg4 arg5 arg6 arg7 arg8


#### Prerequisite: 1. you need covg and read length info per colour. This is printed to stdout by Cortex when running.
...
### Prerequisite:  2. I don't want to have to parse a Cortex output file in R, so I have a script make_covg_file.pl which makes an appropriate input file for the classifier.
###                   You do this:  perl /cortex release dir/scripts/analyse_variants/make_covg_file.pl <cortex callfile> <number of colours in graph> <ref colour> (using -1 if no ref) > covg_for_classifier_file


### Prerequisite 3:   Since the classifier is a bit slow when running on millions of variants
###                   this is set up to allow you to run many copies of this in parallel
