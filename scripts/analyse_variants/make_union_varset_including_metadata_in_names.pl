...
#my $kmer_index = $index.".kmer_index";## file lists varnumber --> kmer called at 
#open(KMER_INDEX, ">".$kmer_index)||die("Cannot open $kmer_index");
### fill hash 
open(FILE, $index)||die();
while (<FILE>)
{
...
sub get_smallest_k
{
    my ($name, $str) = @_; ## format 31,12=41,4=51,5  which are kmer,cleaning _ etc
    my @sp = split(/_/, $str);
    my $i;
    my $min = 9999999999;
    ...
    if (scalar(@sp)>1)
    {
	print "This variant: $name was called with identical branches by multiple kmers (or perhaps the same kmer with multiple cleanings),\n";
