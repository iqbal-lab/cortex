...
void print_little_hash_table_signature(FILE * fp,LittleHashTable * little_hash_table){
  ...
  long long number_buckets = little_hash_table->number_buckets;
  int bucket_size = little_hash_table->bucket_size;
  int ksize = little_hash_table->kmer_size;
...
LittleHashTable * little_hash_table_load_from_dump(FILE * fp, int max_rehash_tries){
  ...
  int bucket_size;
  LittleHashTable * little_hash_table = NULL; 
  ...
  if (read>0 &&
	...
	if (read>0){
	  ...
	  if (read>0){
	    ...
	    little_hash_table->unique_kmers = 0;
	    little_hash_table->max_rehash_tries = max_rehash_tries;
	    little_hash_table->number_buckets = number_buckets;
	    little_hash_table->bucket_size   = bucket_size;
	    ...
	    long long i;
	    for(i=0;i<little_hash_table->number_buckets * little_hash_table->bucket_size;i++){
	      read = fread(&(little_hash_table->table[i]),sizeof(GenotypingElement),1,fp);
	      if (i % 1000000 == 0){
		printf(".");
	      }
	      if (read!= 1){
