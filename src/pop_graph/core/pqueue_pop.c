...
void pqueue_traverse_specific_person_or_pop_for_supernode_and_chromosome_overlap_printing(void (*f)(HashTable*, Element *, long*, EdgeArrayType, int, boolean, char**, char**, int*,  int*),
											  ...
											  boolean is_for_testing, char** for_test1, char** for_test2, int* index_for_test1, int* index_for_test2)
{
  ...
  for(i=0;i<pqueue->number_entries;i++)
    {
      f(hash_table, &(pqueue->elements[i]), supernode_count, type, index, is_for_testing, for_test1, for_test2, index_for_test1, index_for_test2);
    }
}
