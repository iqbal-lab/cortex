  ...
  while(fgets(line,MAX_FILENAME_LENGTH, fp) !=NULL)
    {
      ...
      //  exit(1);
      //}
      total_seq_loaded = total_seq_loaded + load_person_as_fasta_from_filename(line, db_graph, people_so_far-1);
...
int load_person_as_fasta_from_filename(char* filename, dBGraph* db_graph, int index)
{
  ...
  while(fgets(line,MAX_FILENAME_LENGTH, fp) !=NULL)
    {
      total_seq_loaded = total_seq_loaded + load_fasta_data_into_graph_for_specific_person_or_population(fp, db_graph, individual_edge_array, index);
    }
