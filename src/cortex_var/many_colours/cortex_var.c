...
int main(int argc, char **argv){
  ...
  */
  ...
  if(cmd_line.input_seq==true)
  {
    ...
    unsigned long long bases_parsed = 0;
    unsigned long long bases_pass_filters_and_loaded = 0;
    ...
    char *se_list_path = there_is_se_data == true ? cmd_line.se_list : NULL;
    char *pe_list_lh_path = NULL, *pe_list_rh_path = NULL;

    if(there_is_pe_data == true)
    {
      pe_list_lh_path = cmd_line.pe_list_lh_mates;
      pe_list_rh_path = cmd_line.pe_list_rh_mates;
    }

    // Load sequence into colour 0
    load_se_and_pe_filelists_into_graph_colour(se_list_path, 
      pe_list_lh_path, pe_list_rh_path,
      ...
      cmd_line.cut_homopolymers, cmd_line.homopolymer_limit,
      cmd_line.quality_score_offset,
      0, db_graph);

    // Update the graph info object
    unsigned long mean_contig_length = calculate_mean(readlen_distrib,
    ...
    printf("Mean read length after filters applied:%d\n", 
     db_graph_info.mean_read_length[0]);
