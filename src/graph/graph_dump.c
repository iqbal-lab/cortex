...
int main(int argc, char **argv){
  ...
  break_homopolymers = (boolean) atoi(argv[15]);//should we break homopolymer runs
  ...
  if (we_are_loading_some_single_ended_data==true)
	...
	if (quality_cut_off>0)
	  {
	    single_seq_length += load_fastq_from_filename_into_graph(filename,&bad_reads, quality_cut_off, &dup_reads_in_single_ended, max_read_length, 
